...
const ARCH_EA = (typeof window !== 'undefined' && window.electronAPI) ? window.electronAPI : null;
async function archReadPath(path) {
  if (ARCH_EA && ARCH_EA.fs && ARCH_EA.fs.read) return await ARCH_EA.fs.read(path);
  const resp = await fetch(`/api/read?path=${encodeURIComponent(path)}`);
  return await resp.json();
}
async function archListPath(path, recursive) {
  if (ARCH_EA && ARCH_EA.fs && ARCH_EA.fs.list) return await ARCH_EA.fs.list(path, recursive);
  const resp = await fetch(`/api/list?path=${encodeURIComponent(path)}&recursive=${recursive ? 'true' : 'false'}`);
  return await resp.json();
}
...
function defaultArchMemory() {
  return {
    ...
    fileOwnership: [],      // { pattern, owner, description } — e.g. { pattern: "backend/*", owner: "neo" }
...
async function buildSymbolGraph(paths) {
  ...
  for (const basePath of scanPaths) {
    try {
      const entries = await archListPath(basePath, true);
      if (!Array.isArray(entries)) continue;
      for (const entry of entries.slice(0, 50)) {
        if (entry.type === 'dir') continue;
        try {
          const fileData = await archReadPath(entry.path);
