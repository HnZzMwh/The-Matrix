  ...
  tools: {
    save_arch_rules: {
      ...
      run: async (args) => {
        ...
        return '// Usage: techStack={...} or moduleBoundary="..." or adr="..." or owner="neo" pattern="backend/*" or debt="..." or schema="Todo" fields=\'[{"name":"id","type":"int"}]\'';
      },
    },
    query_arch: {
    ...
    check_ownership: {
      ...
      run: async (args) => {
        ...
        if (!filePath) return '// Usage: [TOOL: check_ownership path="src/app.js" agent="neo"]';
    ...
    acquire_lock: {
      ...
      run: (args) => {
        ...
        if (!path) return '// Usage: [TOOL: acquire_lock path="backend/app.py" agent="neo"]';
    ...
    release_lock: {
      ...
      run: (args) => {
        const path = args.path || args.file || '';
        const agentId = args.agent || args.id || 'unknown';
        if (!path) return '// Usage: [TOOL: release_lock path="backend/app.py" agent="neo"]';
    ...
    request_ownership: {
      ...
      run: (args) => {
        ...
        if (!filePath || !owner) return '// Usage: [TOOL: request_ownership path="backend/app.py" owner="neo" reason="..."]';
