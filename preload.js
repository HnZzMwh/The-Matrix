...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  fs: {
    ...
    readSafe:   (p)       => ipcRenderer.invoke('fs:resolveRead', p),
    write:      (p, c)    => ipcRenderer.invoke('fs:write', p, c),
    writeSafe:  (p, c)    => ipcRenderer.invoke('fs:resolveWrite', p, c),
  ...
  // ── MCP (Model Context Protocol) subprocess manager ────
  mcp: {
    start:  (id, config) => ipcRenderer.invoke('mcp:start', id, config),
    stop:   (id)          => ipcRenderer.invoke('mcp:stop', id),
    call:   (id, tool, p) => ipcRenderer.invoke('mcp:call', id, tool, p),
    status: (id)          => ipcRenderer.invoke('mcp:status', id),
  },
});
