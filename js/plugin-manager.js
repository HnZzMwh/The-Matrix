...
class PluginManager {
  ...
  async importFromMarkdown(mdContent) {
    ...
    for (const section of sections) {
      ...
      if (codeMatch) {
        try {
          // Dangerous but necessary for dynamic skills: use Function constructor
          // In a production app, this should be sandboxed or use a safer DSL
          const runFn = new Function('args', 'EA', 'ipcRead', 'ipcWrite', 'ipcList', 'ipcRun', `
            return (async () => {
              ${codeMatch[1]}
            })();
          `);
          ...
          tools[name] = {
            desc,
            run: (args) => runFn(args, EA, ipcRead, ipcWrite, ipcList, ipcRun)
          };
        } catch (e) {
